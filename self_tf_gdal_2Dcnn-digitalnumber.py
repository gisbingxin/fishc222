...
def train_cnn():
    ...
    for step in range(10000):  # 学习的次数是，每次学习量是batch(类数*batch_size)
        ...
        if step % 50 == 0:
            ...
            if 0.8 >= acc > 0.7:
                saver.save(sess, "./model/cnn.model-1.ckpt", global_step=step)
            if 0.9 >= acc > 0.8:
                saver.save(sess, "./model/cnn.model-2.ckpt", global_step=step)
                # break
            if acc > 0.9:
                saver.save(sess, "./model/cnn.model-3.ckpt", global_step=step)
                high_acc_num += 1
                if high_acc_num >=5:
                    break
...
def test_cnn(test_xs):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "./model/cnn.model-3.ckpt-650")
...
if __name__ == '__main__':
    ...
    if train:
        ...
        test_xs, test_ys = get_next_batch(x_data, y_data, num_per_class, sample_num, 60)
        print(test_xs.shape)
        x_img = np.reshape(test_xs[0,:,:,:],[band_num,sample_size_Y,sample_size_X])
        strechimg(sample_size_X,sample_size_Y,x_img[0,:,:],x_img[1,:,:],x_img[2,:,:])

...
#     """
#     print("random forest classifier begins")
...
#     is_train=False
#     is_app=True
#     pik_path = 'G:\data for manuscripts\AVIRIS20100517\\random forest\clf.pickle'
#
#     if is_train:
...
#         row_end = row_start + num_per_class - 1
#         col_end = 2
#         x_data, y_data, sample_num, band_num = from_img_train(file_path, excel_file, class_num, num_per_class, row_start,
#                                                         col_start, row_end, col_end)
#         #y_data = np.reshape(y_data, [sample_num, 1])  # 把行向量转换为列向量，以便在后期与x_data合并
#
#         trained_model=random_forest_classifier(x_data,y_data)
...
#         # file_path = 'G:\data for manuscripts\AVIRIS20100517\\random forest\\test.img'
#         im_data, im_width, im_height, im_bands, im_GeoTrans, im_proj=from_img_app(file_path)
#
...
#             if total_size > 500:
#                 start = 0
#                 n = int(total_size / 500)
#                 for batch_i in range(0, n):
#                     # predicted_label.append(test_cnn(app_xs[start:start + 1000], win_size_X, win_size_Y, cnn_model, batch_i))
#                     # predicted_label = sess.run(label_position,
#                     #                            feed_dict={xs: test_xs[start:start + 1000], keep_prob: 1})
#                     predicted_tmp = trained_model.predict(im_data[start:start+500])
...
#                 predicted_tmp = trained_model.predict(im_data[n*500:total_size])
#                 predicted_label.extend(predicted_tmp)
#                 print('shape of predicted_label', np.shape(predicted_label))
#             else:
#                 predicted_label=trained_model.predict(im_data)
#             #print('shape of predict',np.shape(predicted),predicted)
#             label = np.reshape(predicted_label, (im_height, im_width))
#             label = label + 1
#             out_tif = 'G:\data for manuscripts\AVIRIS20100517\\random forest\\f11_rf.tif'
#             writeTiff(label, im_width, im_height, 1, out_tif, im_GeoTrans, im_proj)
#             plt.imshow(label)
#             plt.show()

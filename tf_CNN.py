...
## conv1 layer ##
W_conv1 = weight_variable([7,7, 1,32]) # patch 5x5, in size 1, out size 32
b_conv1 = bias_variable([32])
h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1) # output size 28x28x32
h_pool1 = max_pool_2x2(h_conv1)                                         # output size 14x14x32

## conv2 layer ##
W_conv2 = weight_variable([7,7, 32, 64]) # patch 5x5, in size 32, out size 64
...
for i in range(300):  #学习的次数是300，每次学习量是60个batch
    batch_xs, batch_ys = mnist.train.next_batch(60)

    sess.run(train_step, feed_dict={xs: batch_xs, ys: batch_ys, keep_prob: 0.5})
    #print(sess.run(ys, feed_dict={xs: batch_xs, ys: batch_ys, keep_prob: 0.5}))
    ...
    #     print(sess.run(W_convn))
        print(compute_accuracy(mnist.test.images[:300], mnist.test.labels[:300]))